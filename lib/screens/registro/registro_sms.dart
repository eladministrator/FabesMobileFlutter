...
class RegistroSmsScreen extends StatefulWidget {
  const RegistroSmsScreen({Key? key}) : super(key: key);

  @override
  State<RegistroSmsScreen> createState() => _RegistroSmsScreenState();
}

class _RegistroSmsScreenState extends State<RegistroSmsScreen> {
  @override
                      ...
                      CardContainer(
                          child: Column(
                            children: [
                              ...
                              Text('Paso 3.- Identidad por SMS', style: TextStyle(color: Colors.brown[900], fontSize: 20)),
...
class _RegistroSmsFormState extends State<_RegistroSmsForm> {
  ...
  Widget build(BuildContext context) {
    ...
    final args = ModalRoute.of(context)!.settings.arguments as RegistroSmsArgs;
    ...
    return Form(
        ...
        child: Column(
          children: [
            const Center(child: Text('Hola,le hemos enviado un mensaje de texto (SMS) con el código de verificación de 4 caracteres para confirmar su identidad y mantener segura su información. ', style: TextStyle(color: Colors.brown, fontSize: 15))),
            const SizedBox(height: 8),
            Text('Celular: ${args.celular}', style: const TextStyle(color: Colors.black, fontSize: 18)),
            const SizedBox(height: 8),
            const Center(child: Text('Le recomendamos estar en un lugar donde tenga una buena señal de celular, ya que de lo contrario no le llegará el mensaje.', style: TextStyle(color: Colors.brown, fontSize: 15))),
            ...
            TextFormField(
              ...
              decoration: InputDecorationsUI.loginInputDecoration(labelText: 'Código', hintText: '0000', prefixIcon: Icons.security),
            ...
            ElevatedButton(
                style: ElevatedButton.styleFrom(primary: const Color.fromARGB(255, 118, 88, 68), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))),
                // Con esta instruccion en base al getter se bloquea al boton
                onPressed: loginForm.isLoading
                    ? null
                    : () {
                        ...
                        if (!loginForm.isValidForm()) {
                          Platform.isIOS ? menssagesUI.displayDialogIos(context, "fabes", "Por favor ingrese un código de 4 números") : menssagesUI.displayDialogAndroid(context, "fabes", "Por favor ingrese un código de 4 números");
                          return;
                        }
                ...
                child: SizedBox(width: double.infinity, child: Center(child: Text(loginForm.isLoading ? 'validando usuario ...' : 'Verificar')))),
